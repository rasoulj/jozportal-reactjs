...
const getDisplayString = (sub, title, locale = "en") => {
    if(sub[0] === ":") return title;

    return Tran(sub, locale);
    //
    // const arr = sub.split("-");
    // if (arr.length > 1) {
...
const ContainerHeader = ({title, match, locale = "en"}) => {
            ...
            <Breadcrumb className="mb-0" tag="nav">
                {subPath.map((sub, index) => {
                        return <BreadcrumbItem active={subPath.length === index + 1}
                                               ...
                                               href={getUrlString(path, sub, index)}>{getDisplayString(sub, title, locale)}</BreadcrumbItem>
                    }
                )}
            </Breadcrumb>
        </div>
